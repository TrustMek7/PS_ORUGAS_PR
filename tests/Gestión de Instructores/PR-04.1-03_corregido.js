...
export default function () {
  ...
  // Paso 2: Rechazar solicitudes masivamente usando POST con payload vacío
  ...
  for (let i = 0; i < solicitudes.length; i++) {
    ...
    const postUrl = `https://teammates-orugas.appspot.com/webapi/account/request/rejection?id=${solicitudId}`;
    const payload = '{}'; // objeto vacío como string

    const postRes = http.post(postUrl, payload, { headers: getHeadersWithCSRF() });

    if (postRes.status === 200 || postRes.status === 204) {
      rechazosExitosos++;
    } else {
      errores++;
      if (errores <= 3) console.log(`❌ Error solicitud ${i + 1}: Status ${postRes.status}`);
    }
